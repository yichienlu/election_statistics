...
<template>
  <div  class="lg:w-[260px] px-5 py-3 border-2 rounded-lg whitespace-nowrap mb-5" :style="{backgroundColor:areaStore.data_county[0].candidate.party_color.light, borderColor:areaStore.data_county[0].candidate.party_color.normal}">
    <h3 class="font-bold text-xl mb-3">{{ areaStore.data.counties[areaStore.selectedCounty].name }}</h3>
    <table style="width:100%">
      <tr class=""  v-for="item in areaStore.data_county" :key="item.no">
        ...
        <td class="pl-5 mb-3">
          <div class="font-bold">{{ (item.votes/ areaStore.data.counties[areaStore.selectedCounty].votes_total * 100).toFixed(1) }} %</div>
